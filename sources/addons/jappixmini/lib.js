...
function jappixmini_addon_get_client_secret(callback) {
	...
	if (client_secret===null) {
		...
		if (!div) {
			div = $('<div id="jappixmini-password-query-div" style="position:fixed;padding:1em;background-color:#F00;color:#fff;top:50px;left:650px;">Retype your Hubzilla password for chatting:<br></div>');
...
function jappixmini_manage_roster(contacts, contacts_hash, autoapprove, autosubscribe) {
	...
	con.registerHandler('presence',function(presence){
		...
		if (autoapprove && contacts[xid]!==undefined) {
			...
			approve = true;
			console.log("Approve known Hubzilla contact "+xid+".");
		}
		else if (autoapprove && pstatus && pstatus.indexOf("Hubzilla")!=-1) {
			...
			approve = false;
			console.log("Do not approve unknown Hubzilla contact "+xid+" - wait instead.");
		}
		else {
	...
	con.send(get_roster, function(iq){
		...
		$(handleXML).find('item').each(function() {
			...
			if ($.inArray("Hubzilla", groups)==-1) {
				group_missing = true;
				groups.push("Hubzilla");
			}

			if (group_missing || name!=contacts[xid]) {
				sendRoster(xid, null, contacts[xid], groups);
				console.log("Added "+xid+" to Hubzilla group and set name to "+contacts[xid]+".");
			}
		...
		for (var xid in contacts) {if(!contacts.hasOwnProperty(xid)) continue;
			...
			// must contain the word "~Hubzilla" so the other side knows
			// how to handle this
			presence.setStatus("I'm "+MINI_NICKNAME+" from ~Hubzilla.\n[machine-generated message]");
			...
			item.appendChild(iq.buildNode('group', {'xmlns': NS_ROSTER}, "Hubzilla"));
			con.send(iq);
